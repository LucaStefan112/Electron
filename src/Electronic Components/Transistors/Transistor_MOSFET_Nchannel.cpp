...
void Transistor_MOSFET_Nchannel::Show(){
    ...
    for(int i = 0; i <= height / 65; i++)
        Transistor_MOSFET_Nchannel_Helper.rotationalCircle(
            center_x + thisHeight / 9, center_y, getPositionCenter(),  2 * height / 9 - i,
            rotateState);

    Transistor_MOSFET_Nchannel_Helper.rotationalLine(
    ...
    Transistor_MOSFET_Nchannel_Helper.rotationalLine(
        down_right_x - thisHeight / 6, down_right_y - height / 20, down_right_x - thisHeight / 6, center_y, getPositionCenter(),
        rotateState);
    ...
    Transistor_MOSFET_Nchannel_Helper.rotationalFillPoly(
        3, points, getPositionCenter(),
        rotateState);

    settextstyle(8, HORIZ_DIR, 3);
    if(height / 50 > 2){
        Transistor_MOSFET_Nchannel_Helper.rotationalOuttextxy(
            down_right_x - thisHeight / 6 - thisHeight / 5 - 5, up_left_y + height / 40, getPositionCenter(), "D",
            rotateState);

        Transistor_MOSFET_Nchannel_Helper.rotationalOuttextxy(
            up_left_x + thisHeight / 7, center_y - height / 10, getPositionCenter(), "G",
            rotateState);
...
void Transistor_MOSFET_Nchannel::updateConnectionPointsPosition(){
    double up_left_x = getPositionUpLeft().x, up_left_y = getPositionUpLeft().y;
    double center_x = getPositionCenter().x, center_y = getPositionCenter().y;
    double down_right_x = getPositionDownRight().x, down_right_y = getPositionDownRight().y;
    ...
    connectionPoints[0].position = Transistor_MOSFET_Nchannel_Helper.rotatePointToReference(
        Transistor_MOSFET_Nchannel_Helper.makeVector_2D(up_left_x + thisHeight / 30, center_y + height / 8),
        getPositionCenter(),
        rotateState);

    connectionPoints[1].position = Transistor_MOSFET_Nchannel_Helper.rotatePointToReference(
        Transistor_MOSFET_Nchannel_Helper.makeVector_2D(down_right_x - thisHeight / 6, up_left_y + height / 30),
        getPositionCenter(),
        rotateState);

    connectionPoints[2].position = Transistor_MOSFET_Nchannel_Helper.rotatePointToReference(
        Transistor_MOSFET_Nchannel_Helper.makeVector_2D(down_right_x - thisHeight / 6, down_right_y - height / 30),
        getPositionCenter(),
        rotateState);
}
