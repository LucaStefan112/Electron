...
void Diode_Constant_Current::Show()
{
    ...
    Diode_Constant_Current_Helper.rotationalCircle(
        up_left_x + thisHeight / 10, center_y, getPositionCenter(), height / 10,
        rotateState);

    Diode_Constant_Current_Helper.rotationalLine(
    ...
    Diode_Constant_Current_Helper.rotationalLine(
        center_x + thisHeight / 2 - thisHeight / 3, up_left_y, center_x + thisHeight / 2 + thisHeight / 3, up_left_y, getPositionCenter(),
        rotateState);

    Diode_Constant_Current_Helper.rotationalLine(
        center_x + thisHeight / 2 - thisHeight / 3, down_right_y, center_x + thisHeight / 2 + thisHeight / 3, down_right_y, getPositionCenter(),
        rotateState);
...
void Diode_Constant_Current::updateConnectionPointsPosition()
{
    ...
    connectionPoints[0].position = Diode_Constant_Current_Helper.rotatePointToReference(
                                       Diode_Constant_Current_Helper.makeVector_2D(up_left_x + thisHeight / 10, center_y),
                                       getPositionCenter(),
                                       rotateState);

    connectionPoints[1].position = Diode_Constant_Current_Helper.rotatePointToReference(
                                       Diode_Constant_Current_Helper.makeVector_2D(down_right_x - thisHeight / 10, center_y),
                                       getPositionCenter(),
                                       rotateState);
}
