...
void Diode_Varactor::Show()
{
    ...
    Diode_Varactor_Helper.rotationalCircle(
        up_left_x + thisHeight / 10, center_y, getPositionCenter(), height / 10,
        rotateState);

    Diode_Varactor_Helper.rotationalLine(
    ...
    Diode_Varactor_Helper.rotationalLine(
        center_x + thisHeight / 2 + thisHeight / 5, up_left_y, center_x + thisHeight / 2 + thisHeight / 5, down_right_y, getPositionCenter(),
        rotateState);

    Diode_Varactor_Helper.rotationalLine(
        center_x + thisHeight / 2 + thisHeight / 5, center_y, down_right_x - thisHeight / 5, center_y, getPositionCenter(),
        rotateState);
...
void Diode_Varactor::updateConnectionPointsPosition()
{
    ...
    connectionPoints[0].position = Diode_Varactor_Helper.rotatePointToReference(
                                       Diode_Varactor_Helper.makeVector_2D(up_left_x + thisHeight / 10, center_y),
                                       getPositionCenter(),
                                       rotateState);

    connectionPoints[1].position = Diode_Varactor_Helper.rotatePointToReference(
                                       Diode_Varactor_Helper.makeVector_2D(down_right_x - thisHeight / 10, center_y),
                                       getPositionCenter(),
                                       rotateState);
}
